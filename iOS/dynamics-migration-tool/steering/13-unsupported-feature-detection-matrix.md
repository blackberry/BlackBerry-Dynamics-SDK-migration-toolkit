...
| SwiftData persistent history / same-store Core Data mix | `fetchHistory`, `deleteHistory`, `HistoryProviding`, one store URL shared with `NSPersistentStoreCoordinator` | Secure store does not support history tracking or dual-stack sharing | Block those patterns; migrate ordinary `@Model` stores via Prompt 04c |
