...
def empty_state(platform: str, run_id: str, budgets: Dict[str, int], maturity_level: str, controlled_prompts: List[str]) -> Dict[str, Any]:
    return {
        ...
        "safety": {
            ...
            "finalAcceptanceOwnedBy": "record-prompt-execution.sh",
            "finalAcceptancePrompts": ["10"],
        },
        "attempts": [],
        "diagnosticCounts": {},
        "promptCounts": {},
...
def task_markdown(task: Dict[str, Any]) -> str:
    ...
    boundary = task.get("controlPlaneBoundary") if isinstance(task.get("controlPlaneBoundary"), dict) else {}
    acceptance_owner = str(boundary.get("finalAcceptanceOwnedBy") or "record-prompt-execution.sh")
    lines = [
        "# Bounded Repair Task",
        "",
        ...
        "",
        f"- Repair orchestrator lane: `{task.get('maturityLevel') or MATURITY_LEVEL}` bounded Stage 7 subset only",
        f"- Final acceptance owner: `{acceptance_owner}`",
        "- Prompt 10 and final source/report acceptance are recorder-owned and not delegated to this orchestrator.",
        "",
        "## Evidence",
        "",
...
def record_failure(args: argparse.Namespace) -> int:
    ...
    task = {
        ...
        "controlPlaneBoundary": {
            "stage": "7",
            "scope": "controlled-subset",
            "finalAcceptanceOwnedBy": "record-prompt-execution.sh",
            "finalAcceptancePrompts": ["10"],
            "notes": "Repair orchestrator creates bounded tasks only; recorder remains authoritative for final acceptance.",
        },
    }
