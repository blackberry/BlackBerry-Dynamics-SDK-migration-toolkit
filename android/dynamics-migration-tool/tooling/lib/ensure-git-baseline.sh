#!/usr/bin/env bash
# BlackBerry Dynamics Migration — consented Git baseline helper.
#
# This helper is called only after 00pre has received explicit developer
# consent. It creates a Git repository when absent, records local excludes for
# migration-tool artifacts, and creates the initial app-source baseline commit
# when the repository has no commits.
#
# It intentionally never edits global/local git config. If `git commit` fails
# because user.name/user.email are unset, the developer must configure Git and
# rerun 00pre.
