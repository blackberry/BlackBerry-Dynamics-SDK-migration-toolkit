...
| `MediaRecorder` file/path output | `MediaRecorder.setOutputFile(path\|file)` or `setNextOutputFile(path\|file)` | `MANUAL_INTERVENTION_REQUIRED` | Prompt 05c: seekable FD bridge (`MemoryFile` / proxy) then GD streams. Do not treat `gdFile.path` as migrated. | Phase 4 rules 4H.mediarecorder + 4I; prompt 05c / `41-secure-media.md` |
| `MediaRecorder` MPEG-4 (or omitted format) on a pipe FD | `createPipe` / `createReliablePipe` + `OutputFormat.MPEG_4` / `THREE_GPP` / `WEBM` / no `setOutputFormat` | `REPLACE_WITH_DYNAMICS` | Non-seekable FD; `MediaRecorder.start()` fails. Use MemoryFile/proxy + GD copy. Sequential `AAC_ADTS`/AMR pipes remain allowed. | Phase 4 rule 4L (`mpeg4-nonseekable-fd`) |
| `MediaPlayer` / `VideoView` / ExoPlayer file path | `setDataSource(path)`, `setVideoPath`, `Uri.fromFile`, ExoPlayer `FileDataSource` | `REPLACE_WITH_DYNAMICS` | GD `absolutePath` is virtual. Use `setDataSource(FileDescriptor)` from the 05c bridge. | Phase 4 rule 4H.mediaplayer-path |
| `MediaMetadataRetriever` / `ThumbnailUtils` file path | `setDataSource(path\|File)`, `createVideoThumbnail(File\|path)` | `REPLACE_WITH_DYNAMICS` | Same FD/stream bridge as playback. | Phase 4 rule 4H.retriever-path |
| CameraX video `FileOutputOptions` / `MediaStoreOutputOptions` | `androidx.camera.video.FileOutputOptions`, `MediaStoreOutputOptions` | `MANUAL_INTERVENTION_REQUIRED` | Video muxers need a seekable FD (`FileDescriptorOutputOptions` on the 05c bridge) or blocking no-go. MediaStore output is an external-storage blocker. | Phase 4 rule 4H.camerax-video |
